...
class ImageDegrader(object):
    ...
    def get_airy_psf(psf_width_pixels, psf_width_meters, z, wavelength, numerical_aperture, refractive_index, normalize=True):
        ...
        if psf_width_pixels % 2 == 0:
            raise ValueError('psf_width_pixels must be an odd number, but is %d.' % psf_width_pixels)
        ...
        for i in range(psf_width_pixels):
            for j in range(psf_width_pixels):
                ...
                psf[i,j]=_evaluate_airy_function_at_point(x, y, z, wavelength, numerical_aperture, refractive_index)
    ...
    def degrade_images(images,output_path,z_depth_meters,exposure_factor,
                       random_seed,photoelectron_factor,sensor_offset_in_photoelectrons,
                       wavelength=500e-9, numerical_aperture=0.5, refractive_index=1.0,
                       psf_width_pixels=51, pixel_size_meters=0.65e-6, 
