...
"""
Run modle inference to annotate input images with path/image predictions.
"""
import logging
import os
import sys
...
class ImageQualityClassifier(object):
    ...
    def _probabilities_from_image(self,image_placeholder,
        ...
        Args:
            ...
            model_patch_side_length:Interger, the side lenght in pixels of
            the square image passed to the model.
            num_classes:Interger, the number of classes the model predicts.
        Returns:
        ...
        model_metrics = evaluation.get_model_and_metrics(tiles, 
    ...
    def predict(self,image):
        ...
        [np_probabilities] = self._sess.run([self._probabilities],feed_dict=feed_dict)
    ...
    def get_patch_predictions(self,image):
        ...
        for i in range(0,image.shape[0]-w,w):
            for j in range(0,image.shape[1]-2,w):
    ...
    def get_annotated_prediction(self,image):
        ...
        with self.graph.as_default():
            patches = _get_image_tiles_tensor(self._image_placeholder,
                                              tensorflow.constant([0]),
                                              tensorflow.constant([0]),
                                              patch_width=self._model_patch_side_length)[0]
            [np_probabilities,np_patches] = self._sess.run([self._probabilities,patches],feed_dict=feed_dict)
            
        # we use '-1' to denote no true label exists.
        np_labels = -1*numpy.ones((np_patches.shape[0]))
        return numpy.squeeze(
            evaluation.visualize_image_predictions(np_patches, 
                                                   np_probabilities, 
                                                   np_labels, 
                                                   image.shape[0], 
                                                   image.shape[1],
...
def patch_values_to_mask(values,patch_width):
        ...
        mask = numpy.zeros((patches_per_column*patch_width,patches_per_row*patch_width),dtype=numpy.uint16)
...
def save_masks_and_annotated_visualization(orig_name,
    ...
    def pad_and_save_image(im,image_output_path):
        ...
        pad_size = ((0,y_pad),(0,x_pad)) if is_greyscale_mask else ((0,y_pad),(0,x_pad),(0,0))
...
def _get_image_tiles_tensor(image,label,image_path,patch_width):
    ...
    tiles_before_reshape = tensorflow.extract_image_patches(tensorflow.expand_dims(image,dim=0), [1,patch_width,patch_width,1], [1,patch_width,patch_width,1], [1,1,1,1], 'VALID')
...
def run_model_inference( model_ckpt_file, probabilities, labels, images,
    ...
    with tensorflow.Session() as sess:
        ...
        for i in range(num_samples):
            ...
            (prediction, certainties, probabilities_i) = evaluation.aggregate_prediction_from_probabilities(np_probabilities, aggregation_method)
        ...
        evaluation.save_inference_results(aggregate_probabilities, aggregate_labels, all_certainties, orig_names, aggregate_predictions, output_file)

        # If we're not sharding, save out accuracy statistics.
        if num_shards == 1:
            save_confusion = not numpy.any(numpy.asarray(aggregate_labels) < 0)

            evaluation.save_result_plots(aggregate_probabilities, aggregate_labels, save_confusion, output_directory, patch_probabilities, patch_labels)
